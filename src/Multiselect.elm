...
input (Model model) =
    let
        ...
        inputDivAttribute =
            let
                preventEscapePropagation =
                    if model.status == Opened then
                        [ preventPropagationButtons ]

                    else
                        []
            in
            [ preventDefaultButtons, css [ SelectCss.inputWrap ] ] ++ preventEscapePropagation
    in
    div
        inputDivAttribute
...
preventPropagationButtons : Html.Styled.Attribute Msg
preventPropagationButtons =
    let
        options =
